...
/* Column layout for each company sheet — matches the structure of
   ...
   hasNotes: true means there's an extra column (titled "Notes" or "LOA")
   between Events and Tuesday, shifting the day columns over by one — set
   it to match whatever that company's sheet currently has.
   If a company's sheet changes shape (more/fewer platoons, that extra
   column added/removed), update its entry here. */
...
async function loadAllData(urls){
  const [muskRows, strelRows, legionRows, hcRows, recRows, depotRows, historyRows] = await Promise.all([
  ...
  return { companies, highCommand, recruitment, depot, history };
}
