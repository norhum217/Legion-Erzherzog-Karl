...
function parsePlatoonBlock(rows, rosterCol, hasNotes, rowStart, rowEnd){
  ...
  for(let r = rowStart; r < end; r++){
    ...
    const notes = hasNotes ? cell(rows, r, rosterCol + 9) : '';
    members.push({
...
function parseCompanySheet(rows, config){
  ...
  return {
    ...
    loaTracked: config.blocks.some(b => !!b.hasNotes),
    platoons,
  };
}
...
/* Maps a history entry's unitTag to one of our 3 companies. The history log
   is server-wide (everyone who fought that day, any regiment), so only tags
   containing "EKL" (Erzherzog Karl Legion) are ours at all. Sub-unit words:
   Fusilier = Musketeers, Schutzen = Strelci, Grenadier = Legionáři.
   Anything else EKL-tagged (Jager officers, staff, artillery attached, etc.)
   counts as "ours" for total attendance but not for the per-company chart. */
function unitTagToCompany(tag){
  if(!tag || !tag.includes('EKL')) return null;
  if(/Fusilier|Fus(?![a-z])/i.test(tag)) return 'musketyri';
  if(/Sch[uü]tzen/i.test(tag)) return 'strelci';
  if(/Grenadier/i.test(tag)) return 'legion';
  return 'other';
}

if(typeof module !== 'undefined'){
  module.exports.parseHistory = parseHistory;
  module.exports.unitTagToCompany = unitTagToCompany;
}
